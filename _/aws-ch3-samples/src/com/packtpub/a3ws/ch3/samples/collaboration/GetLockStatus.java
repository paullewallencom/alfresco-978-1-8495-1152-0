...
public class GetLockStatus {
	...
	public static void main(String[] args) throws RepositoryFault, RemoteException {
		...
		try {
	        ...
	        //Checkin External
	        Reference checkedInNode = authoringService.checkinExternal(
	        		workingCopy.getNodes(0), comments, false, formatForCheckIn, externalContent.getBytes());
	        
	        Predicate predicateToCheckLock = new Predicate(new Reference[]{checkedInNode}, spacesStore, null);
	        
	        //Create a new version    
	        VersionResult versionResult = authoringService.createVersion(predicateToCheckLock,comments, false);
	
	        System.out.println("Created a new version with label: " + versionResult.getVersions(0).getLabel());
	        
	        //Lock the node
	        Reference[] lockedNodes = authoringService.lock(predicateToCheckLock,false, LockTypeEnum.read);
	        ...
	        //Unlock the node
	        Reference[] unlockedNodes = authoringService.unlock(predicateToCheckLock, false);
	...
	private static void checkLockStatus(AuthoringServiceSoapBindingStub authoringService, Predicate predicate) throws AuthoringFault, RemoteException{
