...
public class Unlock {
	...
	public static void main(String[] args) throws RepositoryFault, RemoteException {
		...
		try {
	        ...
	        Predicate predicateToUnlock = new Predicate(new Reference[]{checkedInNode}, spacesStore, null);
	        
	        //Create a new version    
	        VersionResult versionResult = authoringService.createVersion(predicateToUnlock,comments, false);
	
	        System.out.println("Created a new version with label: " + versionResult.getVersions(0).getLabel());
	        
	        //Lock the node
	        Reference[] lockedNodes = authoringService.lock(predicateToUnlock,false, LockTypeEnum.write);
	        
	        System.out.println("Node locked: " + lockedNodes[0].getPath());
	       
	        Reference[] unlockedNodes = authoringService.unlock(predicateToUnlock, false);
