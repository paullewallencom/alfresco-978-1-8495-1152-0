...
public class GetACLs {
	...
	public static void main(String[] args) throws AccessControlFault, RemoteException {
		...
		try {
			...
			//getACLs
			ACE filter = new ACE();
			filter.setAuthority("GROUP");
			filter.setPermission(Constants.ALL_AUTHORITIES);
			filter.setAccessStatus(AccessStatus.acepted);
			ACL[] acls = accessControlService.getACLs(predicate, filter);
			for (ACL aclElement : acls) {
				ACE[] acesElement =  aclElement.getAces();
				for (ACE aceElement : acesElement) {
					System.out.println("getACLs | Authority: "+aceElement.getAuthority() +"| Permission: "+aceElement.getPermission());
				}
			}
