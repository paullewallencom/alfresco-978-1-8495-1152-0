...
public class Lock {
	...
	public static void main(String[] args) throws RepositoryFault, RemoteException {
		...
		try {
	        ...
	        Predicate predicateToLock = new Predicate(new Reference[]{checkedInNode}, spacesStore, null);
	        
	        //Create a new version    
	        VersionResult versionResult = authoringService.createVersion(predicateToLock,comments, false);
	
	        System.out.println("Created a new version with label: " + versionResult.getVersions(0).getLabel());
	        
	        //Lock the node
	        Reference[] lockedNodes = authoringService.lock(predicateToLock, false, LockTypeEnum.write);
