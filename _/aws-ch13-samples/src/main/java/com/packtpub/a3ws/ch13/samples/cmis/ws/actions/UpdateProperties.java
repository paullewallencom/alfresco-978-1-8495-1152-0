...
public class UpdateProperties {
	...
	private static Log log = LogFactory.getLog(UpdateProperties.class);
	...
	public static void main(String[] args) throws CmisException {
		...
		objectService.createDocument(repositoryId, newDocProperties, rootFolderId, stream, null, null, null, null, null, idHolder);
		log.info("Document created. Id: "+idHolder.value);
		
		//creating the new name property and update properties
		Holder<String> changeToken = new Holder<String>();
		CmisPropertiesType newUpdatedDocProperties = new CmisPropertiesType();
		
		CmisPropertyString nameUpdatedDoc = objectFactory.createCmisPropertyString();
		nameUpdatedDoc.setPropertyDefinitionId("cmis:name");
		nameUpdatedDoc.getValue().add("CMIS WS API Sample Document - UPDATED - " + System.currentTimeMillis());
		newUpdatedDocProperties.getProperty().add(nameUpdatedDoc);
		
		objectService.updateProperties(repositoryId, idHolder, changeToken, newUpdatedDocProperties, null);
